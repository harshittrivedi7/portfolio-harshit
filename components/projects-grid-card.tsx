...
export function ProjectsGridCard() {
  return (
    <div>
      <h2 className="mb-6 text-2xl font-bold tracking-tight text-foreground">Additional Projects</h2>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {otherProjects.map((project, index) => (
          <motion.a
            ...
            className="group relative overflow-hidden rounded-xl border border-border bg-card p-6 transition-all duration-300 hover:border-primary/50 hover:shadow-lg hover:shadow-primary/10"
          >
            ...
            <div className="relative z-10 flex flex-col h-full">
              <div className="flex items-start justify-between gap-3 mb-3">
                <h3 className="font-semibold text-foreground text-base leading-tight flex-1">{project.title}</h3>
                <ExternalLink className="w-4 h-4 text-muted-foreground flex-shrink-0 transition-colors group-hover:text-primary" />
              </div>

              <p className="mb-4 text-sm text-muted-foreground leading-relaxed flex-1">{project.description}</p>

              <div className="flex flex-wrap gap-2 pt-4 border-t border-border/50">
                {project.tech.map((tech) => (
                  <span
                    key={tech}
                    className="text-xs rounded-full bg-primary/15 text-primary px-2.5 py-1 border border-primary/30 transition-all group-hover:bg-primary/25 group-hover:border-primary/50"
                  >
                    {tech}
                  </span>
                ))}
